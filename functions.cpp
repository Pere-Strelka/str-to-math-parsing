...
namespace stmr
{
    ...
    template <class T>
    void operator+=(std::vector<T>& first, std::vector<T>& second)
    {
    ...
    Operator defineFunction(std::string& function)
    {
        if (function == "negate")
    ...
    std::vector<Operation> findFunctions(std::string& str, unsigned int& opCount)
    {
        ...
        for (int i = 0; i < strCopy.length(); i++) {
            if (std::islower(strCopy[i])) {
                ...
                std::vector<Operation> FO = findOperations(strCopy, opCount);
                arr += FO;
    ...
    std::vector<Operation> findOperations(std::string& str, unsigned int& opCount)
    {
        ...
        std::vector<Operation> FB = findBraces(str, opCount);
        array += FB;
        ...
        for (int i = 1; i < str.length(); i++) {
            if (str[i] == '*' || str[i] == '/') {
                ...
                if (str[i - 1] == '}') { // if there is another operation on the left, we'll get its index
                    ...
                    for (leftSymbolOfOperation = i - 1; str[leftSymbolOfOperation] != '-' &&
                        str[leftSymbolOfOperation] != '+' &&
